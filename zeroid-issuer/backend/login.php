...
try {
    ...
    if ($action === 'client-first') {
        ...
        $stmt = $conn->prepare("
            SELECT u.id, u.username, u.token, u.issuer_id,
                   u.scram_salt, u.scram_iterations, u.scram_stored_key, u.scram_server_key,
                   i.name AS issuer_name, i.did AS issuer_did,
        ...
        echo json_encode([
            ...
            'data' => [
                ...
                'issuer_did'           => $user['issuer_did'],
                'issuer_eth_address'   => $user['issuer_eth_address'],
