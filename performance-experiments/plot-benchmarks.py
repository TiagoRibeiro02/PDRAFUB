...
def plot_nft_purchase_composition(nft_runs: list[dict], out_dir: Path) -> None:
    ...
    steps = [
        ("NFT Purchase", purchase, "increase"),
        ("Transfer to DID", transfer, "increase"),
        ("DID Linking", link_did, "increase"),
        ("Sequential Workflow Total", 0, "total"),
        ("Optimized Flow", delta, "decrease"),
        ("Integrated Workflow Total", 0, "total"),
    ]
    ...
    for label, value, kind in steps:
        ...
        if kind == "increase":
            ...
            starts.append(cumulative - value)
            heights.append(value)
            colors.append("#DA3F3F")
            cumulative -= value
        else:  # total
            starts.append(0)
