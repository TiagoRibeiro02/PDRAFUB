<?php
/**
 * bank1_api.php  –  Bank1 user-lookup API
 *
 * Served at: http://localhost:8002/bank1_api.php
 * (registered in zeroid_entity.entities.API for the Bank1 entity)
 *
 * GET  ?action=search&q=<query>   → search bank1 users by name or NIF
 * GET  ?action=get&id=<id>        → fetch a single bank1 user by id
 * POST { action:"set_kyc", userId:<id> } → mark a bank1 user as KYC-verified
 */
...
require_once __DIR__ . '/bank1_db.php';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'set_kyc') {
        ...
        if ($stmt->rowCount() === 0) {
            http_response_code(404);
            echo json_encode(['success' => false, 'message' => 'User not found in bank1']);
            exit();
        }
